...
parser = argparse.ArgumentParser(description="Evaluate logical reasoning of responses using DeepSeek as a judge.")
...
def get_logic_evaluation(response, test_response):
    """
    Evaluate the logic of the model-generated response.
...
for lang in selected_languages:
    ...
    output_csv_path = output_dir / f"{lang}_qwen2.5_1.5B_logic_evaluation.csv"
    ...
    for idx in tqdm(unevaluated_indices):
        ...
        if not all(isinstance(x, str) and x.strip() for x in [response, test_response]):
    ...
    print(f"Completed logic evaluation for {lang}. Results saved to: {output_csv_path}")
